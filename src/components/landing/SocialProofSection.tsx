      ...
      <div className="relative max-w-7xl mx-auto px-6">
        <FadeUp>
          <div className="text-center mb-16">
            <h2 className="text-3xl sm:text-4xl md:text-5xl font-extrabold tracking-tight mb-4 text-[hsl(var(--landing-fg))]">
              Pessoas reais,{" "}
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {testimonials.map((t, i) => (
            <FadeUp key={t.name} delay={0.1 * (i + 1)}>
              <div className="rounded-3xl border border-[hsl(var(--landing-card-border))] bg-[hsl(var(--landing-card))] p-7 sm:p-8 h-full flex flex-col transition-all hover:border-primary/20">
                <div className="flex gap-1 mb-4">
                  {Array.from({ length: 5 }).map((_, j) => (
                    <Star key={j} className="w-4 h-4 fill-amber-400 text-amber-400" />
                  ))}
                </div>
                <p className="text-sm sm:text-base text-[hsl(var(--landing-muted))] leading-relaxed flex-1 mb-6">
                  "{t.text}"
                </p>
                <div className="flex items-center gap-3">
                  <img src={t.avatar} alt={t.name} className="w-10 h-10 rounded-full object-cover" loading="lazy" />
                  <div>
                    <p className="text-sm font-semibold text-[hsl(var(--landing-fg))]">{t.name}</p>
                    <p className="text-xs text-[hsl(var(--landing-muted))]">{t.age}</p>
                  </div>
                </div>
              </div>
            </FadeUp>
